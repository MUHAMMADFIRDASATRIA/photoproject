...
import { getStoredTheme, storeTheme, themeClass, Theme } from '../lib/theme';
...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div className={`relative flex min-h-screen items-center justify-center overflow-hidden bg-zinc-950 font-sans text-zinc-100 selection:bg-indigo-500 selection:text-white ${themeClass(theme)}`}>
      {/* Toggle Light / Dark Mode */}
      <button
        onClick={handleToggleTheme}
        title={theme === 'light' ? 'Ganti ke Dark Mode' : 'Ganti ke Light Mode'}
        className="absolute right-5 top-5 z-20 flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900/60 text-zinc-300 backdrop-blur-xl transition hover:bg-zinc-800 hover:text-amber-400"
      >
        {theme === 'light' ? (
